...
# def first_n_primes(n):
#     return islice(filter(prime, count(start = 2)), n)

# #sha512 constants

# def fractional(f, n = 64):
#     """ returns the first n bits of fractional part of float"""
#     f -= math.floor(f) # get only the fractional part
#     f *= 2**n # shifting left
#     f = int(f) # truncate the rest of the fractional part
#     return f

# def generateK():

#     return [fractional(p ** (1/3.0)) for p in first_n_primes(80)]

# def generateHash():
#     """
#     Follows Section 5.3.5 to generate the initial hash value H^0
#     The first 64 bits of the fractional parts of the square roots of
#     the first 8 prime numbers.
    
#     """
#     return [fractional(p ** (1/2.0)) for p in first_n_primes(8)]
...
#     b.append(0x80) # appending 10000000 in binary (=128 in decimal)

#     # follow by K zero bits, where k is the smallest, non-negative solution to the equation
#     # l +1+ k ≡ 896mod 1024 . Then append the 128-bit block that is equal to the number l expressed
#     # using a binary representation.
#     while (len(b)* 8) % 128 != 112:
#         b.append(0x00)

#     # the last 128-bit block is the length l of the original message
#     # expressed in binary (big endian, msb is at the beginning of the byte array)
#     b.extend(l.to_bytes(8, 'big'))
...
#     # b = padding(b)
#     for offset in range(0, len(b), 128):
#         blocks = b[offset : offset + 128]
#         W = [0 for t in range(80)]
#         for t in range(16):
#             W[t] = int.from_bytes(blocks[8*t : 8*t + 8], byteorder='big')

#         for t in range(16, 80):
#             term1 = omega1(b2i(W[t-2]))
#             term2 = b2i(W[t-7])
#             term3 = omega0(b2i(W[t-15]))
#             term4 = b2i(W[t-16])
#             total = (term1 + term2 + term3 + term4) % 2**64
#             # W.append(i2b(total))

#         # 2. Initialize the 8 working variables a,b,c,d,e,f,g,h with prev hash value
#         a, b, c, d, e, f, g, h = H

#         # 3. Hash value of  64 bit words
#         for t in range(80):
#             T1 = (h + sigma1(e) + ch(e, f, g) + K[t] + b2i(W[t])) % 2**64
...
#         # 4. Compute the i-th intermediate hash value H^i
#         delta = [a, b, c, d, e, f, g, h]
#         H = [(i1 + i2) % 2**64 for i1, i2 in zip(H, delta)]
